...
export class PagosService {
  constructor(
    ...
    private readonly pagoCuotaRepo: Repository<PagoCuota>,
  ...
  async findPagosByCliente(userId: number, rol: number) {
    if (rol !== 3) {
      throw new ForbiddenException(
        'Solo los clientes pueden acceder a esta información',
      );
    }
  ...
  async eliminarPago(id: number): Promise<{ message: string }> {
