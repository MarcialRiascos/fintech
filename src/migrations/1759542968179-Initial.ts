...
export class Initial1759542968179 implements MigrationInterface {
    name = 'Initial1759542968179'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`cuentas_cobro\` (\`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`id\` int NOT NULL AUTO_INCREMENT, \`total\` decimal(15,2) NOT NULL DEFAULT '0.00', \`fecha_corte\` date NOT NULL, \`mes\` int NOT NULL, \`anio\` int NOT NULL, \`tiendas_id\` int NULL, \`estados_id\` int UNSIGNED NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`ALTER TABLE \`orden_compra\` ADD \`cuentas_cobro_id\` int NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`img_tiendas\` ADD CONSTRAINT \`FK_de574cc0de8f1f56c34db1dcffa\` FOREIGN KEY (\`tiendas_id\`) REFERENCES \`tiendas\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`orden_compra\` DROP COLUMN \`cuentas_cobro_id\``);
        await queryRunner.query(`DROP TABLE \`cuentas_cobro\``);
    }
